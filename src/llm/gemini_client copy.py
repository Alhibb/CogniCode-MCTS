...
class GeminiClient(AbstractLLMClient):
    """
    Implementation of the LLM Client using Google's Gemini 2.5.
    ...
    def __init__(self, api_key: str = None, mock_mode: bool = False):
        ...
        self.model_name = 'gemini-2.5-flash' # Flash is faster/cheaper for high-iteration search
        
        if not mock_mode:
            self.api_key = api_key or os.getenv("GEMINI_API_KEY", "AIzaSyDQL4pKIzE-MMIME5IxUuRWroFj7Y07Mm8")
            if not self.api_key:
